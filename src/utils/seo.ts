...
export function generateArticleKeywords(article: Partial<Article>): string {
  ...
  ['SGA News', 'berita terkini', 'portal berita'].forEach(brand => {
...
export function updateArticleSEO(
  ...
  if (activeView === 'detail' && article) {
    ...
    updateMetaTag('meta[property="article:published_time"]', 'property', 'article:published_time', article.publishedAt || article.createdAt);
    ...
    updateMetaTag('meta[name="title"]', 'name', 'title', pageTitle);
    updateMetaTag('meta[name="description"]', 'name', 'description', description);
    updateMetaTag('meta[name="keywords"]', 'name', 'keywords', 'SGA News, berita indonesia, berita terkini, jurnalistik, portal berita');
